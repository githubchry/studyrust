...
fn main() {
    {
        ...
        // 变量以被创建时相反的顺序被丢弃，所以 b 在 a 之前被丢弃。
    ...
    */
    {
        ...
        println!("CustomSmartPointers created.");
        // c.drop();   // 编译错误：不允许显式调用 drop 析构函数（destructor）
        ...
        std::mem::drop 位于 prelude，所以可以直接调用：
        */
        drop(c);
        println!("CustomSmartPointers dropped before the end of scope.");
    }

    /*
    Drop trait 实现中指定的代码可以用于许多方面，来使得清理变得方便和安全：
    比如可以用其创建我们自己的内存分配器！通过 Drop trait 和 Rust 所有权系统，你无需担心之后的代码清理，Rust 会自动考虑这些问题。
